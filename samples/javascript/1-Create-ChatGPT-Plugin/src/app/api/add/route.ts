...
export async function GET(req: NextRequest) {     
    ...
    // if req is not null, return sum of the two numbers
    const sum = parseInt(number1 as string) + parseInt(number2 as string);
    console.log("Sum -- ", sum);

    return NextResponse.json({
        sum: sum,
        infoLink: `http://localhost:9080/details?number1=${number1}&number2=${number2}&result=${sum}&method=add`,
    }, { 
        status: 200, 
        headers: {
