...
> [!TIP]
> Key topics: (1) Tokens: basic units of text/code for LLM AI models to process/generate language. (2) Tokenization: splitting input/output texts into smaller units for LLM AI models. (3) Vocabulary size: the number of tokens each model uses, which varies among different GPT models. (4) Tokenization cost: affects the memory and computational resources that a model needs, which influences the cost and performance of running an OpenAI or Azure OpenAI model. _This list of key topics was produced with the semantic skill `SummarizeSkill.Topics`_
