...
`ContextProvider` is an abstract class that you can inherit from, and which can be associated with the `AgentThread` for a `ChatAgent`.
It allows you to:
...
The `ContextProvider` class has two main methods that you can override to run custom logic before and after the agent invokes the underlying inference service:
...
The `ContextProvider` class also supports thread lifecycle events:

- `thread_created` - called when a new thread is created. This can be used to initialize any thread-specific state or data.

### Sample ContextProvider implementation

Let's look at an example of a custom memory component that remembers a user's name and age, and provides it to the agent before each invocation.

First we'll create a model class to hold the memories using Pydantic:
...
1. It uses a `ChatClientProtocol` to look for the user's name and age in user messages when new messages are added to the thread at the end of each run.
1. It provides any current memories to the agent before each invocation.
...
1. It also implements a basic serialization method to allow persisting the memories.

```python
from collections.abc import MutableSequence, Sequence
from typing import Any

from agent_framework import ChatClientProtocol, ChatMessage, ChatOptions, Context, ContextProvider
from pydantic import BaseModel
...
class UserInfoMemory(ContextProvider):
    def __init__(self, chat_client: ChatClientProtocol, user_info: UserInfo | None = None, **kwargs: Any):
        ...
        Args:
            ...
            user_info: Initial user information if available.
            **kwargs: If provided, will be used to create a UserInfo object.
        """
...
To use the custom `ContextProvider`, you provide it directly when creating the agent using the `context_providers` parameter. This parameter accepts a single provider, a list of providers, or an `AggregateContextProvider`.

When creating a `ChatAgent` you can provide the `context_providers` parameter along with all other agent options:
...
async def main():
    async with AzureCliCredential() as credential:
        ...
        async with ChatAgent(
            ...
            # Agent is ready to use with memory
            pass
```

You can also use the `create_agent` factory method available on chat clients:

```python
from agent_framework.openai import OpenAIChatClient

# Create chat client
client = OpenAIChatClient(api_key="your-api-key")
...
When creating a new thread, the `ContextProvider` will be automatically attached to the thread. Once memories are extracted it is therefore possible to access the memory component via the thread's `context_provider` property and inspect the memories:
...
# Access the memory component via the thread's context_provider property
if thread.context_provider and hasattr(thread.context_provider, 'providers'):
    user_info_memory = thread.context_provider.providers[0]  # First provider in the aggregate
    if hasattr(user_info_memory, 'user_info'):
...
The Python Agent Framework supports using multiple context providers simultaneously. When multiple providers are specified, they are automatically wrapped in an `AggregateContextProvider` that coordinates their execution:

```python
# Create multiple context providers
memory_provider = UserInfoMemory(chat_client)
another_provider = SomeOtherContextProvider()

# Create agent with multiple providers
agent = ChatAgent(
    chat_client=chat_client,
    instructions="You are a helpful assistant.",
    context_providers=[memory_provider, another_provider]  # List of providers
)
```

When multiple providers are used:
- All providers' `invoking` methods are called, and their contexts are merged
- All providers' `invoked` methods are called after the agent response
- All providers' `thread_created` methods are called when a new thread is created
